...
var swekey_integration_params =
{
    ...
    'logo_green_str'    : 'Swekey plugged and authentified',
    'logo_green_str'    : 'Swekey plugged and validated',
    'attach_ask_str'	: "A swekey authentication key has been detected.\nDo you want to associate it with your account ?",
...
function cb_ajax_test(result)
{
    ...
    {
// We do not need multiple level anymore (jquery limitation)
//    	if (result.arr.length == 2)
		   	alert("Ajax test was successful");
//		else
//		   	alert("Ajax test failed with arrays:" + result.arr);
	}
}
...
function cb_swekey_validate(result)
{
    ...
    {
        if (swekey_retry < 2) // we retry twice (session problems)
        {
	        swekey_retry ++;
            swekey_ids = "";
...
function swekey_login_onload()
{
    ...
    if (swekey_loginnames_input.length > 0)
    {
		for (var i = 0; i < swekey_loginnames_input.length; i++)
	    {
	    	...
	    	swekey_logos[i].setAttribute('style', 'width:8px; height:16px; padding:0px 2px 0px 2px; border-spacing:0px; margin:0px; vspace:0px; hspace:0px; frameborder:no; vertical-align:middle;'
			+ 'position:relative;  left:' + swekey_integration_params.logo_xoffset + '; top:' + swekey_integration_params.logo_yoffset + ';');
...
function swekey_propose_to_attach()
{
	...
	if (id != "")
	{
    	...
    	document.cookie = "swekey_proposed=" + id + "; path=/;";   // call it only once
